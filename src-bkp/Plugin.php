...
class Plugin
{
	...
	public function run()
	{
		...
		$this->loadClasses(
			[
				...
				// \SayHello\Plugin\Icon\Plugin\Package\Icon::class,
				// \SayHello\Plugin\Icon\Plugin\Package\OptionsPage::class,
				// \SayHello\Plugin\Icon\Plugin\Package\Settings::class,
	...
	{
		register_rest_route(shp_icon()->prefix . '/v1', '/icons/', [
		'methods'  => 'GET',
		'callback' => function ($data) {
			...
			foreach ($icons as $key => $icon) {
				array_push($icon_list, [
					'name' => ucwords(shp_icon()->Package->Icon->getIconNameFromFileName($icon)),
					'filename' => $icon,
